...
The gallery now lives under `/gallery/` in an npm-workspaces monorepo (see the
root `/CLAUDE.md` and `/PROGRESS.md`). What changed for the gallery:

- **Shared primitives moved out to `@rsvis/math`** (`/math/src/`): `ode`,
  `quaternion`, `quadrature`, `winding`, `rng` — together with their tests
  (`ode/quaternion/quadrature/winding` test files moved to `/math/test/`; the
  24 primitive tests now run there). All gallery imports of these were rewritten
  from local paths to `import { … } from '@rsvis/math'`.
- **What remains local to the gallery** (`gallery/src/math/`): the
  widget-specific kernels — `clutching`, `monopole`, `hopf`, `surfaces`,
  `degree`, `instanton`, `mobius` — and their tests (43 tests in
  `gallery/test/`). These are applications of the shared primitives, specific to
  the R&S widgets, so they stay here rather than in the shared package.
- **Nothing left to migrate that is cleanly shareable.** The widget kernels do
  not import each other, so the split was clean. If the forthcoming `bundle`
  project turns out to need any widget kernel (e.g. the Hopf or quaternion-degree
  geometry), promote that specific kernel into `@rsvis/math` with its tests at
  that point — do not pre-emptively move it now.
- Gallery test count is unchanged in total: 24 (now in `@rsvis/math`) + 43 (here)
  = 67, all green. Build green from the new location; `base` unchanged
  (`/R-S-Visualizations/`, gallery at the site root).
